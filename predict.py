...
class TensorTransformer:
    ...
    def letterbox(self, image: cv2.Mat, new_size: int = 224) -> cv2.Mat:
        ...
        background = numpy.ones((new_size, new_size, 3), dtype = numpy.uint8) * 127
...
class FlowerClassifier:
    ...
    def predict(self, image: cv2.Mat) -> tuple[str, float]:
        input_tensor = self.transformer.to_tensor(image)
        with torch.no_grad():
    ...
    def test_image_dir(self, images_path: str = "./images", result_path: str = "./results") -> None:
        ...
        print("\n---------- Testing Image Directory Finished ----------\n")
